...
impl SpotifyConnector {
    ...
    pub async fn connect_new() -> Result<SpotifyConnector> {
        ...
        connector.update_port().await;
        // Connect to SpotifyWebHelper and start Spotify.
        if let Err(error) = connector.start_spotify().await {
            // The connection failed, error out.
            return Err(error);
        }
        // Fetch the OAuth token.
        connector.oauth_token = match connector.fetch_oauth_token().await {
            Ok(result) => result,
            Err(error) => return Err(error),
        };
        // Fetch the CSRF token.
        connector.csrf_token = match connector.fetch_csrf_token().await {
            Ok(result) => result,
            Err(error) => return Err(error),
        };
    ...
    async fn start_spotify(&self) -> Result<bool> {
        match self
            .query(&self.get_local_url(), REQUEST_OPEN, false, false, None)
            .await
        {
    ...
    async fn fetch_oauth_token(&self) -> Result<String> {
        let json = match self.query(URL_TOKEN, "", false, false, None).await {
            Ok(result) => result,
            Err(error) => return Err(error),
        };
        match json["t"].as_str() {
    ...
    async fn fetch_csrf_token(&self) -> Result<String> {
        let json = match self
            .query(&self.get_local_url(), REQUEST_CSRF, false, false, None)
            .await
        {
            Ok(result) => result,
            Err(error) => return Err(error),
    ...
    pub async fn fetch_status_json(&self) -> Result<JsonValue> {
        self.query(&self.get_local_url(), REQUEST_STATUS, true, true, None)
            .await
    }
    /// Requests a track to be played.
    pub async fn request_play(&self, track: String) -> bool {
    ...
    pub async fn request_pause(&self, pause: bool) -> bool {
    ...
    async fn query(
        ...
        let response = {
            let resp = match self
                .client
                .get::<&str>(url.as_ref())
                .header(USER_AGENT, format!("{}", HEADER_UA))
                .header(
                    ORIGIN,
