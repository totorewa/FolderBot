        ...
        if let Ok(a) = caps.str_at(1).parse::<i64>() {
            if let Ok(b) = caps.str_at(3).parse::<i64>() {
                return match caps.get(2).unwrap().as_str() {
                    "*" => a.checked_mul(b).map_or("Um... no, but nice try.", |v| v).to_string(),
                    "/" => a.checked_div(b).map_or("152. xD", |v| v).to_string(),
                    "-" => a.checked_sub(b).map_or("...why.", |v| v).to_string(),
                    "+" => a.checked_add(b).map_or("Great work, you rolled a 255!", |v| v).to_string(),
                    _ => "Unknown...".to_string(),
                };
            }
        }
